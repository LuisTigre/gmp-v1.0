          ...
          <tr v-for="(titulo,index) in titulos[1]">
            ...
            <td v-bind:id="'segunda_'+parseInt(index+7)" v-on:drop="drop" v-on:dragover="allowDrop">
             ...
             <template v-for="item in lista.alocadas">
              <form v-bind:id="'frm_segunda_'+parseInt(index+7)" v-if="editar && token" v-bind:action="editar + item.id" method="post">

              <input type="hidden" name="_method" value="put">
              <input type="hidden" name="_token" v-bind:value="token">
              <input type="hidden" name="single_update" value="single_update">
              <input type="hidden" name="tempo_id" v-bind:value="'segunda_'+parseInt(index+7)">
  ...
  export default {
    ...
    methods:{
        ...
        drop:function(ev){
          ...
          let aula_id = ev.target.firstChild.id; 
          console.log(aula_id);           

          axios.put('/admin/aulas/'+aula_id+'update_tempo_id', {tempo_id}).then(res => {  
